...
| `/api/compass` | 1.4 s first per goal | Then cached; charges scale with par (clamp(par-2,1,4)). Static build: compass-lite — `compass/{n}.json` per daily, complete truncated BFS levels (~250k ids/goal, delta-encoded, own depth `d`), generated by `static_export` (`--compass-days`, default 365 + archive), oracle-tested against `distances_to`. |
