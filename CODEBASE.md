...
| `e2e/` | 4 tests | Playwright smoke suite against a DEPLOYED site (`BASE_URL`, default production) — CI can't build the 18 GB tree. Each test is a real playtest regression: give-up must keep the route reveal and never print "could not check", a lagged double-click counts once, random goals carry real in-degrees. `cd e2e && npm ci && npx playwright test`. |
