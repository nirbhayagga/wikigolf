...
| `bin/static_export.rs` | ~280 | Emits the whole game as a static file tree (shards, search buckets, the page, and dailies/rounds pre-generated a year ahead by default — ~2 KB/day, so `--days 3650` is a decade). Puzzle endpoints (dailies, rounds, random pools) carry full start/goal article objects (desc, in-degree, views, x/y) so the goal card and the map's red goal dot match the live server. The page's `data-static` adapter consumes it and derives "today" from the client clock. Simple English: ~20 s; enwiki: ~15 min, 17 GB, ~15k files. |
